...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function register($identity, $password, $email, $additional_data = array(), $groups = array(), $complete = false)
	{
		...
		elseif ( !defined("DEFAULT_GROUP") && empty($groups) )
		{
		...
		$query = $this->db->get_where($this->tables['groups'],array('name' => DEFAULT_GROUP),1)->row();
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select('username, email, id, password, active, last_login, first_name, last_name')
