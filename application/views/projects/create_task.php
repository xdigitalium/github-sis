...
$date_hidden = array(
  'type'         => 'hidden',
  'name'         => 'task[date]',
  'value'        => set_value("task[date]", date_MYSQL_JS(date("Y-m-d"))),
);
$date_due = array(
  "id"           => "date_due",
  "class"        => "form-control"
...
$date_due_hidden = array(
  'type'         => 'hidden',
  'name'         => 'task[date_due]',
  'value'        => set_value("task[date_due]", ""),
);
$hour_rate = array(
  "id"           => 'hour_rate',
  "name"         => 'task[hour_rate]',
  "value"        => set_value('task[hour_rate]', "0"),
  "class"        => "form-control",
  "autocomplete" => "off",
  "type"         => "number",
  "step"         => "any",
...
<hr />
<?php echo form_open("projects/create_task/".$project_id, array('class' => 'form-horizontal'));?>
<?php echo form_hidden('task[project_id]', $project_id); ?>
<div class="bordered_tabs">
  ...
  <div class="tab-content">
    <div class="tab-pane form-horizontal" id="tab_basic">
      <div class="row">
        ...
        <div class="col-md-6">
          ...
          <div class="form-group m-a-0 required">
            ...
            <?php echo form_dropdown('task[priority]', $this->settings_model->getPriorities(), '', 'class="form-control"'); ?>
          </div>
        </div>
      </div>
      <div class="row">
        <div class="col-md-6">
          <!-- HOUR_RATE -->
          <div class="m-a-0 form-group required">
            <?php echo lang('hour_rate', 'hour_rate', $label_params); ?>
            <?php echo form_input($hour_rate); ?>
          </div>
        </div>
        <div class="col-md-6">
          <!-- PRIORITY -->
          <div class="form-group m-a-0 required">
            ...
            <?php echo form_dropdown('task[status]', $this->settings_model->getProjectTaskStatus(), '', 'class="form-control"'); ?>
          </div>
        </div>
      </div>
      <div class="row">
      ...
      <div class="row">
        <div class="col-md-12">
          <textarea class="form-control m-a-0" rows="3" name="task[description]" id="editor_description"><?php echo set_value("task[description]", "") ?></textarea>
        </div>
      </div>
    </div>
...
<script type="text/javascript">
  ...
  $('#task_tabs a[href="#tab_basic"]').tab('show');
  ...
  .on("change", function(){
      if( $(this).datepicker("getDate") != null ){
          $('input[name="task[date]"]').val(date_locale($(this).datepicker("getDate"), globalLang["lang"], "en"));
      }else{
          $('input[name="task[date]"]').val("");
      }
  });
  ...
  .on("change", function(){
      if( $(this).datepicker("getDate") != null ){
          $('input[name="task[date_due]"]').val(date_locale($(this).datepicker("getDate"), globalLang["lang"], "en"));
      }else{
          $('input[name="task[date_due]"]').val("");
      }
  });
  if( $('input[name="task[date]"]').val() != "" ){
      $("#date").datepicker("setDate",date_locale($('input[name="task[date]"]').val(), "en", globalLang["lang"]));
  }else{
  ...
  $.attachments = {
    add: function(file){
      ...
      $('<input name="task[attachments][]" value="'+file.id+'" type="hidden">').appendTo(item);
