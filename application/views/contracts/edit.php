...
$subject = array(
  ...
  'id'           => 'subject',
  'value'        => set_value("contract[subject]", $contract->subject),
  'class'        => "form-control",
...
$biller = array(
  'name'         => 'biller',
  'id'           => 'biller_id',
  'value'        => set_value("biller", $biller->fullname),
  'class'        => "form-control",
);
$biller_id_hidden = array(
  'type'         => 'hidden',
  'name'         => 'contract[biller_id]',
  'value'        => set_value("contract[biller_id]", $contract->biller_id),
);
$amount = array(
  "id"           => 'amount',
  "name"         => 'contract[amount]',
  "value"        => set_value('contract[amount]', $contract->amount),
  "class"        => "form-control",
  "autocomplete" => "off",
  "type"         => "number",
  "step"         => "any",
...
$type = array(
  ...
  "id"           => "type",
  "class"        => "form-control",
  'value'        => set_value("contract[type]", $contract->type),
);
$date = array(
  "id"           => "date",
  "class"        => "form-control"
...
$date_hidden = array(
  'type'         => 'hidden',
  'name'         => 'contract[date]',
  'value'        => set_value("contract[date]", date_MYSQL_JS($contract->date)),
);
$date_due = array(
  "id"           => "date_due",
  "class"        => "form-control"
...
$date_due_hidden = array(
  ...
  'value'        => set_value("contract[date_due]", date_MYSQL_JS($contract->date_due)),
);
?>
...
<hr />
<?php echo form_open("contracts/edit/".$contract->id, array('class' => 'form-horizontal'));?>
<?php echo form_hidden('id', $contract->id); ?>
<?php echo form_hidden('contract[count]', $contract->count); ?>
<?php echo form_hidden('contract[reference]', $contract->reference); ?>
...
<div class="row">
  <div class="col-md-5 col-md-offset-1">
    ...
    <div class="m-a-0 form-group ">
      ...
      <span class="form-control"><?php echo $contract->reference ?></span>
    </div>
  </div>
  <div class="col-md-5">
    <!-- SUBJECT -->
    ...
    <div class="m-a-0 form-group required">
      ...
      echo '<select name="contract[currency]" id="currency" class="form-control">';
        foreach ($this->settings_model->getFormattedCurrencies() as $currency) {
          echo "<option value='".$currency->value."' ".($currency->value==$contract->currency?"selected='selected'":"" ).">".$currency->label."</option>";
        }
      echo "</select>";
      ?>
    </div>
...
<div class="form-group row">
  <div class="col-md-12">
    <textarea class="form-control" rows="3" name="contract[description]" id="editor_description"><?php echo set_value("contract[description]", $contract->description) ?></textarea>
  </div>
</div>
...
<script type="text/javascript">
  ...
  if( $('input[name="contract[date_due]"]').val() != "" ){
      $("#date_due").datepicker("setDate",date_locale($('input[name="contract[date_due]"]').val(), "en", globalLang["lang"]));
  }else{
   ...
   *  BILLER (AUTOCOMPLETE)
   */
  var selected_biller = <?php echo json_encode($biller) ?>;
  $('#biller_id')
  .change(function(){
      if( $(this).val() == "" ){
