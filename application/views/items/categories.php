...
<ol class="breadcrumb">
    ...
    <div class="flip pull-right" style="line-height: 64px;">
        ...
        <a href="<?php echo site_url('/items/create_category/');?>" sis-modal="categories_table" class="btn btn-primary-outline sis_modal tip" title="<?php echo lang("add"); ?>"> <i class="fa fa-plus"></i></a>
        <?php endif ?>
...
$(document).ready(function() {
    ...
    var categories_table = $('#categories_table').dataTable( {
        ...
        'bProcessing'    : true,
        'bServerSide'    : true,
        'sAjaxSource'    : SITE_URL+'/items/getDataCategories',
    ...
    categories_table.on("select-count", function(e){
        ...
        if( select_count > 0 ){
            html = globalLang["selected"]+" <b>"+select_count+"</b> "+globalLang["categor"+(select_count==1?"y":"ies")]+".";
    ...
    $(document).on('click', '.delete_selected', function() {
        if( !$(this).is('.disabled') ){
            ...
            bconfirm(globalLang['alert_confirmation'], function(){
                $('#categories_table').load_ajax(SITE_URL+"/items/delete_category", 'POST', selected_rows);
            });
        }
        return false;
    });
...
function actions_format(data, type, row, meta){
    actions = '<div class="btn-group">'+
                ...
                '</button>'+
                '<ul class="dropdown-menu dropdown-menu-right">';
    actions += $('#categories_table').create_datatable_action("trash", SITE_URL+"/items/delete_category?id="+row.id, globalLang['delete'], false, true, false, true);
    actions += $('#categories_table').create_datatable_action("pencil", SITE_URL+"/items/update_category/"+row.id, globalLang['edit'], false, false, true);
    actions += '</ul></div>';
    return "<center>"+actions+"</center>";
}
function name_format(value, y ,row) {
