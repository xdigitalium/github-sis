...
(function ($) {
    ...
    $.fn.columnFilter = function (options) {
        ...
        function fnCreateInput(oTable, regex, smart, bIsNumber, iFilterLength, iMaxLenght, sName, sClass) {
            ...
            var input = $('<input type="text" class="' + search_init + sCSSClass + '" value="" placeholder="'+label+'"/>');
        ...
        function fnCreateDateRangeInput(oTable, sName, sClass, sDateFormat) {
            ...
            var sCssClass = "dandelion_column_filter dandelion_date_range_filter date-range-input " + sClass;

            th.html("");

            var sDateRangeId = oTable.attr("id") + '_range_date_' + i;
            var dateRangeInput = $('<input type="text" class="' + sCssClass + '" id="' + sDateRangeId + '" rel="' + i + '" placeholder="'+label+'"/>');
            ...
            dateRangeInput.daterangepicker(
                {
                    ...
                    locale: {
                        format: sDateFormat.toUpperCase(),
                        separator: properties.sRangeSeparator
                    }
                }
            );
            ...
            oTable.dataTableExt.afnFiltering.push(
		        function (oSettings, aData, iDataIndex) {
                    ...
                    var dStartDate = $('.date-range').data('daterangepicker').getStartDate();
                    var dEndDate = $('.date-range').data('daterangepicker').getEndDate();
            ...
            if(properties.sFilteringTrigger == "immediate") {
               $('#' + sDateRangeId , th).change(function () {
                    oTable.fnDraw();
                    fnOnFiltered();
               });
            }
        ...
        var default_async = $.ajaxSettings.async;
        $.ajaxSetup({ async: false });
        if ( defaults.sUrl !== "" )
        {
            $.getJSON( defaults.sUrl, null, function( json ) {
        ...
        return this.each(function () {
                ...
                if (aoColumn != null) {
                    ...
                    switch (aoColumn.type) {
                        case "null":
                            break;
                        case "number":
                            fnCreateInput(oTable, true, false, true, aoColumn.iFilterLength, aoColumn.iMaxLenght, aoColumn.sName, properties.classes);
                            break;
                        case "select":
                            if (aoColumn.bRegex != true)
                                aoColumn.bRegex = false;
                            fnCreateSelect(oTable, aoColumn.values, aoColumn.bRegex, aoColumn.selected, aoColumn.sName, properties.classes);
                            break;
                        case "number-range":
                            fnCreateRangeInput(oTable, aoColumn.sName, properties.classes);
                            break;
                        case "date-range":
                            fnCreateDateRangeInput(oTable, aoColumn.sName, properties.classes, properties.sDateFormat);
                            break;
                        case "checkbox":
                        ...
                        default:
                            ...
                            fnCreateInput(oTable, bRegex, bSmart, false, aoColumn.iFilterLength, aoColumn.iMaxLenght, aoColumn.sName, properties.classes);
                            break;
            ...
            for (j = 0; j < aiCustomSearch_Indexes.length; j++) {
                ...
                var fnSearch_ = function () {
                    ...
                    /*_range_from_ = $("#" + id + "_range_from_" + aiCustomSearch_Indexes[j]).val();
                    if( _range_from_ == undefined ) _range_from_ = "";
                    _range_to_ = $("#" + id + "_range_to_" + aiCustomSearch_Indexes[j]).val();
                    if( _range_to_ == undefined ) _range_to_ = "";
                    var s_Search = _range_from_ + properties.sRangeSeparator + _range_to_;*/
